...
class GraphLayer(keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._propagate_kwargs = _has_training_param(self.propagate)
        self.built = False
...
class GraphConv(GraphLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._update_kwargs = _has_training_param(self.update)
    ...
    def propagate(
        ...
        if not self._message_kwargs:
            ...
            message = self.message(tensor, training=training)
        ...
        if not self._aggregate_kwargs:
            ...
            aggregate = self.aggregate(message, training=training)
        ...
        if not self._update_kwargs:
            ...
            update = self.update(aggregate, training=training)
    ...
    for outer_field, data in spec.__dict__.items():
        ...
        for inner_field, nested_spec in data.items():
            if inner_field in ['label', 'sample_weight']:
                ...
                continue
            shape, dtype = nested_spec.shape, nested_spec.dtype
            if not scalar_spec or outer_field != 'context':
                shape = shape[1:]
            kwargs = {
...
def replicate(
    ...
    If the original layer was built, its weights will be transferred as well.
    '''
